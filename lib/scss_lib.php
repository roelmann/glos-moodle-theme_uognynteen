 ...
 */
function theme_uognynteen_css_tree_post_processor($tree, $theme) {
 ...
 * @return string
 */
function theme_uognynteen_get_main_scss_content($theme) {
    global $CFG;

    $scss = '';

    $scss .= file_get_contents($CFG->dirroot . '/theme/uognynteen/scss/defaultvariables.scss');
    ...
    $iterator = new DirectoryIterator($CFG->dirroot . '/theme/uognynteen/scss/preset/');
    $presetisset = '';
    foreach ($iterator as $pfile) {
        if (!$pfile->isDot()) {
            $presetname = substr($pfile, 0, strlen($pfile) - 5); // Name - '.scss'.
            if ($filename == $presetname) {
                $scss .= file_get_contents($CFG->dirroot . '/theme/uognynteen/scss/preset/' . $pfile);
                $presetisset = true;
            }
        }
    }
    if (!$presetisset) {
        $filename .= '.scss';
        if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_uognynteen', 'preset', 0    , '/', $filename))) {
            $scss .= $presetfile->get_content();
        } else {
            // Safety fallback - maybe new installs etc.
            $scss .= file_get_contents($CFG->dirroot . '/theme/uognynteen/scss/preset/default.scss');
        }
    }

    $scss .= file_get_contents($CFG->dirroot . '/theme/uognynteen/scss/styles.scss');
 ...
 * @return array
 */
function theme_uognynteen_get_pre_scss($theme) {
    global $CFG, $OUTPUT;
    ...
    if (isset($headerbg)) {
        ...
        $prescss .= 'header#page-header .card {background-image: url([[pix:theme|banner]]);
 ...
 * @return string
 */
function theme_uognynteen_get_extra_scss($theme) {
