...
class mod_assign_renderer extends \mod_assign_renderer {
    ...
    public function render_assign_grading_summary(\assign_grading_summary $summary) {
        ...
        if ($summary->cutoffdate) {
            ...
            $cell1 = new html_table_cell('Planned feedback return date');
    ...
    public function render_assign_feedback_status(\assign_feedback_status $status) {
        ...
        if (isset($status->gradefordisplay)) {
            ...
            $row->attributes['class'] = 'fbgrade';
            $cell1 = '';
            $cell2 = new html_table_cell(get_string('provisionalgrade', 'theme_uognynteen'));
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
    ...
    public function render_assign_submission_status(\assign_submission_status $status) {
        ...
        $assignduedate = $DB->get_field('assign', 'duedate', array('id' => $status->submission->assignment));
        $time = time();
        ...
        $o .= $this->output->box_start('boxaligncenter submissionsummarytable rotest');
        ...
        if ($status->teamsubmissionenabled) {
            $row = new html_table_row();
            $row->attribute['class'] = "teamsubmission";
        ...
        if ($status->attemptreopenmethod != ASSIGN_ATTEMPT_REOPEN_METHOD_NONE) {
            ...
            $row = new html_table_row();
            $row->attributes['class'] = "attemptnumber";
        ...
        $row = new html_table_row();
        $row->attributes['class'] = 'submissionstatus';
        ...
        // Grading status.
        $row = new html_table_row();
        $row->attributes['class'] = 'gradingstatus';
        ...
        $submission = $status->teamsubmission ? $status->teamsubmission : $status->submission;
        if ($status->teamsubmission) {
            $assignduedate = $DB->get_field('assign', 'duedate', array('id' => $status->teamsubmission->assignment));
        } else {
            $assignduedate = $DB->get_field('assign', 'duedate', array('id' => $status->submission->assignment));
        }
        $duedate = $assignduedate2 = $status->duedate;
        if ($duedate > 0) {
            // Due date.
            $row = new html_table_row();
            $row->attributes['class'] = 'duedate';

            $cell1 = new html_table_cell(get_string('duedate', 'assign'));
            $cell2 = new html_table_cell(userdate($assignduedate));
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
            ...
            if ($status->extensionduedate) {
                // Extension date.
                $row = new html_table_row();
                $row->attributes['class'] = 'extensionduedate';
            ...
            // Time remaining.
            $row = new html_table_row();
            $row->attributes['class'] = 'timeremaining';
        ...
        if ($status->view == assign_submission_status::GRADER_VIEW) {
            $row = new html_table_row();
            $row->attributes['class'] = 'editingstatus';
        ...
        if (!empty($status->gradingcontrollerpreview)) {
            $row = new html_table_row();
            $row->attributes['class'] = 'gradingmethodpreview';
        ...
        if ($submission) {
            $row = new html_table_row();
            $row->attributes['class'] = 'timemodified';
